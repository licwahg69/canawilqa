...
@section('content_header')
    <h1 class="m-0 text-primary text-center"><b>Tipo de Documentos</b></h1>
@stop

@section('content')
<form action="/typedoc" method="POST" id="view" name="view" class="formeli">
    @csrf
    <input type="hidden" id="typedoc_id" name="typedoc_id" value="">
            ...
            <div class="card">
                <div class="card-body">
                        ...
                        <table id="tbank" class="table table-striped table-bordered display responsive nowrap">
                            ...
                            <tbody>
                                @foreach ($type_docs as $type_doc)
                                    <tr>
                                        <td width="30" class="text-left">{{ $type_doc->id }}</td>
                                        <td class="text-left">{{ $type_doc->countryname }}</td>
                                        <td class="text-left">{{ $type_doc->description }}</td>
                                        <input type="hidden" id="nombre{{ $type_doc->id }}"
                                            value="{{ $type_doc->description }}">
                                        <td width="60" class="text-center">
                                            @if ($permissions > 2)
                                                <a href="#" onclick="validar('edit',{{ $type_doc->id }})"
                                                    class="btn btn-xs btn-default text-primary mx-1 shadow" title="Editar {{ $type_doc->description }}">
                                                    <i class="fa fa-lg fa-fw fa-pen"></i>
                                                </a>
                                            @endif
                                            @if ($permissions > 3)
                                                <button type="submit" onclick="validar('delete',{{ $type_doc->id }})"
                                                    class="btn btn-xs btn-default text-danger mx-1 shadow" title="Eliminar {{ $type_doc->description }}">
                    ...
                    <div id="datatable2" style='display: none;'>
                        <table id="tbank2">
                            <tbody>
                                @foreach($type_docs2 as $type_doc2)
                                <tr>
                                    <td>
                                        <div class="row">
                                            <div>
                                    ...
                                    <td>
                                        <div class="row">
                                            ...
                                            <div>
                                                {{ $type_doc2->countryname }}
                                            </div>
                                        </div>
                                    </td>
                                    <input type="hidden" id="nombre{{ $type_doc2->id }}"
                                                value="{{ $type_doc2->bankname }}">
                                    <td>
                                        <div class="row">
                                            <div>
                                                <b>Descripción:</b>
                                            ...
                                            <div>
                                                {{ $type_doc2->description }}
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        @if ($permissions > 2)
                                            <a href="#" onclick="validar('edit',{{ $type_doc2->id }})"
                                                class="btn btn-primary btn-sm" data-bs-toggle="tooltip" data-bs-placement="top"
                                                title="Editar {{ $type_doc2->description }}">Editar <i class='fa fa-pen'></i>
                                            </a>
                                        @endif
                                        @if ($permissions > 3)
                                            <button type="submit" onclick="validar('delete',{{ $type_doc2->id }})"
                                                class="btn btn-danger btn-sm" data-bs-toggle="tooltip"
                                                data-bs-placement="top" title="Eliminar {{ $type_doc2->description }}">Eliminar <i
...
<script>
    ...
    function validar(xaccion, xid) {
        ...
        document.getElementById('typedoc_id').value = xid;
        ...
        if (xaccion == "delete") {
            $('.formeli').submit(function(e) {
                ...
                Swal.fire({
                    title: '¡Confirmar!',
                    text: "¿Realmente desea eliminar el Tipo de Documento " + delnombre + "?",
                    icon: 'warning',
