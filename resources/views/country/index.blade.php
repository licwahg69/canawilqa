...
@section('content_header')
    <h1 class="m-0 text-primary text-center"><b>Paises</b></h1>
@stop

@section('content')
<form action="/country" method="POST" id="view" name="view" class="formeli">
    @csrf
                        ...
                        <table id="tbank" class="table table-striped table-bordered display responsive nowrap">
                            <thead class="bg-dark text-white">
                                <tr>
                                    ...
                                    <th width="60" class="text-center">Código</th>
                            ...
                            <tbody>
                                @foreach ($countries as $country)
                                    <tr>
                                        <td width="30" class="text-left">{{ $country->id }}</td>
                                        <td class="text-left">{{ $country->countryname }}</td>
                                        <input type="hidden" id="nombre{{ $country->id }}"
                                            value="{{ $country->countryname }}">
                                        <td width="60" class="text-left">{{ $country->phone_code }}</td>
                                        <td width="60" class="text-center">
                                            @if ($permissions > 2)
                                                <a href="#" onclick="validar('edit',{{ $country->id }})"
                                                    class="btn btn-xs btn-default text-primary mx-1 shadow" title="Editar {{ $country->countryname }}">
                                                    <i class="fa fa-lg fa-fw fa-pen"></i>
                                                </a>
                                            @endif
                                            @if ($permissions > 3)
                                                <button type="submit" onclick="validar('delete',{{ $country->id }})"
                                                    class="btn btn-xs btn-default text-danger mx-1 shadow" title="Eliminar {{ $country->countryname }}">
                        ...
                        <table id="tbank2">
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <input type="hidden" id="nombre{{ $country2->id }}"
                                                value="{{ $country2->countryname }}">
                                    <td>
                                        <div class="row">
                                            <div>
                                                <b>Código:</b>
                                    ...
                                    <td>
                                        @if ($permissions > 2)
                                            <a href="#" onclick="validar('edit',{{ $country2->id }})"
                                                class="btn btn-primary btn-sm" data-bs-toggle="tooltip" data-bs-placement="top"
                                                title="Editar {{ $country2->countryname }}">Editar <i class='fa fa-pen'></i>
                                            </a>
                                        @endif
                                        @if ($permissions > 3)
                                            <button type="submit" onclick="validar('delete',{{ $country2->id }})"
                                                class="btn btn-danger btn-sm" data-bs-toggle="tooltip"
                                                data-bs-placement="top" title="Eliminar {{ $country2->countryname }}">Eliminar <i
...
<script>
    ...
    function validar(xaccion, xid) {
        ...
        document.getElementById('country_id').value = xid;
        ...
        if (xaccion == "delete") {
            $('.formeli').submit(function(e) {
                ...
                Swal.fire({
                    title: '¡Confirmar!',
                    text: "¿Realmente desea eliminar el País " + delnombre + "?",
                    icon: 'warning',
