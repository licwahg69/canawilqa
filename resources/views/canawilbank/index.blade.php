...
@section('title', 'Bancos de Canawil')
...
@section('content_header')
    <h1 class="m-0 text-primary text-center"><b>Bancos de Canawil</b></h1>
@stop

@section('content')
<form action="/canawilbank" method="POST" id="view" name="view" class="formeli">
    @csrf
    <input type="hidden" id="canawilbank_id" name="canawilbank_id" value="">
                        ...
                        <table id="tbank" class="table table-striped table-bordered display responsive nowrap">
                            <thead class="bg-dark text-white">
                                <tr>
                                    ...
                                    <th width="40" class="text-center">Cuenta</th>
                            ...
                            <tbody>
                                @foreach ($banks as $bank)
                                    <tr>
                                        ...
                                        <td width="40" class="text-left">{{ $bank->account_number }}</td>
                                        <td width="60" class="text-center">
                                            @if ($permissions > 2)
                                                <a href="#" onclick="validar('edit',{{ $bank->id }})"
                                                    class="btn btn-xs btn-default text-primary mx-1 shadow" title="Editar {{ $bank->bank_name }}">
                                                    <i class="fa fa-lg fa-fw fa-pen"></i>
                                                </a>
                                            @endif
                                            @if ($permissions > 3)
                                                <button type="submit" onclick="validar('delete',{{ $bank->id }})"
                                                    class="btn btn-xs btn-default text-danger mx-1 shadow" title="Eliminar {{ $bank->bank_name }}">
                                    ...
                                    <td>
                                        @if ($permissions > 2)
                                            <a href="#" onclick="validar('edit',{{ $bank2->id }})"
                                                ...
                                                title="Editar {{ $bank2->bank_name }}">Editar <i class='fa fa-pen'></i>
                                            </a>
                                        @endif
                                        @if ($permissions > 3)
                                            <button type="submit" onclick="validar('delete',{{ $bank2->id }})"
                                                class="btn btn-danger btn-sm" data-bs-toggle="tooltip"
                                                data-bs-placement="top" title="Eliminar {{ $bank2->bank_name }}">Eliminar <i
...
<script>
    ...
    function validar(xaccion, xid) {
        ...
        document.getElementById('canawilbank_id').value = xid;
        ...
        if (xaccion == "delete") {
            $('.formeli').submit(function(e) {
                ...
                Swal.fire({
                    title: '¡Confirmar!',
                    text: "¿Realmente desea eliminar el Banco de Canawil " + delnombre + "?",
                    icon: 'warning',
