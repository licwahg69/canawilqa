...
@section('content_header')
    <h1 class="m-0 text-primary text-center"><b>Editar Divisa: </b><label class="text-dark">{{$currencies->description}}</label></h1>
@stop

@section('content')
<form action="/currency" method="POST" id="view" name="view">
    @csrf
    <input type="hidden" id="currency_id" name="currency_id" value="{{$currencies->id}}">
                    ...
                    <div class='row'>
                        <div class="col-md-4 form-group">
                            ...
                            <select id="country_id" name="country_id" class="form-control" tabindex="1" onchange="quitaMensaje()" autofocus>
                                ...
                                @else
                                    @foreach ($countries as $country)
                                        @if ($currencies->country_id == $country->id)
                                            <option value="{{$country->id}}" selected>{{$country->countryname}}</option>
                                        @else
                                            <option value="{{$country->id}}">{{$country->countryname}}</option>
                                        @endif
                        ...
                        <div class="col-md-4 form-group">
                            ...
                            <input type="text" class="form-control" id="description" name="description" onkeypress="quitaMensaje()" maxlength="50" value="{{ old('description') ?? $currencies->description ?? old('description') }}" placeholder="Descripción" tabindex="2">
                        ...
                        <div class="col-md-2 form-group">
                            ...
                            <input type="text" class="form-control" id="currency" name="currency" onkeypress="quitaMensaje()" onkeyup="aMayusculas(this.value,this.id)" maxlength="3" value="{{ old('currency') ?? $currencies->currency ?? old('currency') }}" placeholder="Moneda" tabindex="3">
                        ...
                        <div class="col-md-2 form-group">
                            ...
                            <input type="text" class="form-control" id="symbol" name="symbol" onkeypress="quitaMensaje()" maxlength="3" value="{{ old('symbol') ?? $currencies->symbol ?? old('symbol') }}" placeholder="Símbolo" tabindex="4">
