                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-md-6 form-group">
                            ...
                            <div class="input-group mb-3">
                                <input class="form-control" type="text" id="payer" name="payer" placeholder="Nombre del pagador" value="" onkeydown="quitaMensaje()">
                                <button class="btn btn-primary btn-sm height" type="button" onclick="searchPayer()">
                                    Buscar
                                </button>
                            </div>
                            <div id="payer_error" class="talert" style='display: none;'>
                                <p class="text-danger">El Nombre del pagador es requerido</p>
                            </div>
                            <div id="payer_error2" class="talert" style='display: none;'>
                                <p class="text-danger">No hay una cuenta agregada</p>
                            </div>
                        </div>
                        <div class="col-md-6 form-group">
                            <label for="cellphone" class="form-label"><b>Celular del Pagador (*):</b></label>
                            <div class="input-group mb-3">
                                <span class="input-group-text height"><i class="bi bi-whatsapp"></i></span>
                                <select id="whatsapp_code" name="whatsapp_code" class="form-control" onclick="quitaMensaje()">
                                    <option value="">Seleccionar</option>
                                    @foreach ($country_phone as $phone)
                                        <option value="{{$phone->phone_code}}">{{$phone->country}}</option>
                                    @endforeach
                                </select>
                            ...
                            <div id="whatsapp_code_error" class="talert" style='display: none;'>
                                <p class="text-danger">El código Internacional del Celular es requerido</p>
                            </div>
        ...
        <div class="modal fade" id="myModal5" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg modal-dialog-scrollable">
                <div class="modal-content">
                        ...
                        <div class="container-fluid">
                            <div class="row">
                                <div class="col-md-12 form-group">
                                    <div class="mm-dropdowns">
                                        <div class="mm-dropdown">
                                            <button class="mm-dropdown-toggle"><b>Nombre de Pagadores grabados de este Aliado Comercial</b></button>
                                            <div class="mm-dropdown-menu" id="nombrePayerButton">
                                                <input type="text" id="nombrePayerFilter" class="mm-dropdown-filter" placeholder="Filtrar">
                                                <a href="#" class="mm-dropdown-option">Seleccionar</a>
                                            </div>
                                            <input type="hidden" id="selectedNombrePayer" name="selectedNombrePayer" value="">
                                        </div>
                                    </div>
                                    <div id="delete_payer_error" class="talert" style='display: none;'>
                                        <p class="text-danger">Debe seleccionar el pagador que desa eliminar</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                        <button type="button" class="btn btn-primary" data-bs-dismiss="modal" onclick="seleNomPayer()">Seleccionar</button>
                        <button type="button" class="btn btn-danger" onclick="deletePayer()">Eliminar</button>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    function mostrarPayer(jsondata){
        document.getElementById("payer").value = jsondata.payer_name;
        document.getElementById("whatsapp_code").value = jsondata.phone_code;
        document.getElementById("cellphone").value = jsondata.onlynumber;
        $('#buttongrabar').focus();
    }
    ...
    function deletePayer(){
        var xseguir = true;
        var xselectedNombrePayer = document.getElementById("selectedNombrePayer").value;
        if (xselectedNombrePayer.length < 1){
            xseguir = false;
            document.getElementById("delete_payer_error").style.display = "block";
        }
        if (xseguir){
            fetch(`/delete_payer/${xselectedNombrePayer}`)
    ...
    function searchPayer(){
        fetch(`/findnombrepayer`)
            .then(response => response.json())
            .then(jsondata => showNombrePayer(jsondata))
    }

    function showNombrePayer(jsondata){
        let objState_id = document.getElementById('nombrePayerButton');
        clearSelect(objState_id);

        // Añadir el campo de entrada para filtrar
        let filterInput = document.createElement('input');
        filterInput.type = 'text';
        filterInput.id = 'nombrePayerFilter';
        filterInput.className = 'mm-dropdown-filter';
        filterInput.placeholder = 'Buscar';
        filterInput.addEventListener('input', function() {
            filterOptions('nombrePayerFilter');
        });
        objState_id.appendChild(filterInput);
        ...
        jsondata.forEach(element => {
            ...
            optionTag.href = "#";
            optionTag.className = "mm-dropdown-option";
            optionTag.innerHTML = element.payer_name + ' ' + element.cellphone;
            optionTag.addEventListener('click', function(event) {
                event.preventDefault();
                objState_id.querySelectorAll('.mm-dropdown-option').forEach(opt => opt.classList.remove('selected'));
                this.classList.add('selected');
                document.getElementById('selectedNombrePayer').value = element.id;
            });
    ...
    function validar(){
        ...
        var xpayer = document.getElementById("payer").value;
        if (xpayer.length < 1){
            xseguir = false;
            document.getElementById("payer_error").style.display = "block";
        }
        ...
        var xphonecode = document.getElementById("whatsapp_code").value;
        if (xphonecode.length < 1){
            xseguir = false;
            document.getElementById("whatsapp_code_error").style.display = "block";
        }
